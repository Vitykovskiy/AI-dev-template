...
- Epic: a large initiative that moves through intake, analysis, delivery, deploy, and e2e validation.
...
- Contour: a delivery boundary with its own owner, such as `frontend`, `backend`, `devops`, or `qa-e2e`.
...
- Done: delivered for the current stage, persisted in canonical artifacts, reflected in GitHub state, and advanced to the next allowed lifecycle step.
- Initiative complete: contour delivery is finished, deployment succeeded, and e2e validation passed.
- Blocked: cannot continue because of an unresolved dependency, missing access, or missing specification that requires return to a previous stage.
