...
- when implementation is blocked by missing specification, record a linked `system_analysis` follow-up row or issue instead of local assumptions
...
| `<task>` | `design` | `designer` | `<block task or n/a>` | `<dependencies>` | `<analysis artifacts>` | `<result>` |
| `<task>` | `implementation` | `<designer/frontend/backend/devops/qa-e2e>` | `<block_delivery task>` | `<dependencies>` | `<analysis and design artifacts>` | `<result>` |
